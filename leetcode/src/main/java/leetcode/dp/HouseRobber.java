...
public class HouseRobber {
    ...
    // Integer[] memo;
    public int rob_4r(int[] nums) {
        memo = new Integer[nums.length];
        return helper_4r(nums, 0);
    }

    private int helper_4r(int[] nums, int index) {
        if (index >= nums.length) {
            return 0;
        }
        ...
        int with = helper_4r(nums, index + 2) + nums[index];
        int without = helper_4r(nums, index + 1);
